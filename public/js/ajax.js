    ...
    $('#seeFriends').on('click', function() {
        ...
        $.ajax({
            ...
            function(json) {
                ...
                for( var i = 0; i < json.length; i++)
                {
                    content += '<div class="single-list-item"><div class="image-box single-list-image"><a href=""><img class="backup_picture" src="'+json[i].foto+'" alt="Imagen de ejemplo"></a></div><div class="single-list-text"><a href="#"><span> ' + json[i].email_user2 +'  </span></a><div class="party-options"><a title="Borrar amigo" href=""><i class="fa fa-trash"></i></a></div></div></div>';
                }
                content += '';
                $('.single-list').html(content);
        }).fail(
